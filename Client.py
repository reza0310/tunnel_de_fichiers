...
def debut():
    global client_socket, t_IP, t_PORT, Fen, txt_etat
    Fen = tk.Tk()
    Fen.title("Tunnel de données (client)")
...
def connexion():
    try:
        ...
        if e == "[WinError 10056] Une demande de connexion a été effectuée sur un socket déjà connecté":
            client_socket.close()
        else:
...
def co_reu():
    ...
    try:
        path_env = path + "\\Desktop\\Envoyer (Client TDD)"
        path_rec = path + "\\Desktop\\Recevoir (Client TDD)"
        os.mkdir(path_env)  # mkdir = make directory
        os.mkdir(path_rec)
        txt_etat.config(text="Bureau localisé.")
    except:
        try:
            path_env = path + "\\Bureau\\Envoyer (Client TDD)"
            path_rec = path + "\\Bureau\\Recevoir (Client TDD)"
            os.mkdir(path_env)
            os.mkdir(path_rec)
            txt_etat.config(text="Bureau localisé.")
        except:
            try:
                path_env = path + "\\OneDrive\\Bureau\\Envoyer (Client TDD)"
                path_rec = path + "\\OneDrive\\Bureau\\Recevoir (Client TDD)"
                os.mkdir(path_env)
                os.mkdir(path_rec)
                txt_etat.config(text="Bureau localisé.")
            except:
